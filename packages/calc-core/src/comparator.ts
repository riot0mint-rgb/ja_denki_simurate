...
export interface DiscountTerms {
  /**
   * ガスとでんきのセット割（円/月）。適用外なら 0。
   *
   * **正の値で渡す。** 試算表 I20 は -110 と書かれているが、あちらは
   * 請求額に足す向きの符号。ここは削減額に足すので 110 を渡す。
   * 符号を取り違えると年額が 110×12×2 = 2,640円 ずれる。
   */
  gasSetDiscountMonthly: Decimal;
  /**
   * 初年度のみの特別割引（円）。**正の値で渡す**（試算表 I25 は -3,000）。
   */
