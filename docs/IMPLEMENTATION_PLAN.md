...
### フェーズ8〜11（2026-08-21 に順序変更、2026-09-07 に6-b・10-bを追加）

| フェーズ | 内容 | 状態 |
|---|---|---|
| **6-b** | 燃料費調整額・再エネ賦課金の自動取得（完全自動） | 🚧 着手（2026-09-07・ユーザー承認） |
| **8** | 説明の自動生成（AI案内役） | ✅ 完了（テンプレート方式・2026-08-21） |
| **8-b** | 商談ナビ（営業支援）| ✅ 完了（2026-08-21・テンプレート方式） |
| **8-c** | 商談の確度判定・再訪の管理・集計の管理者向け切り離し | ✅ 完了（2026-08-22） |
| **9** | セキュリティレビュー・受け入れテスト | ✅ 完了（2026-08-21、2026-08-22 に55項目へ拡張） |
| **10** | ローンチ（配布方法と運用手順の確定） | 着手（オンプレミス VM 前提で DEPLOY.md 3-4／3-5 を追加） |
| **10-b** | WEB申し込み手続きへの接続 | 📝 設計中（2026-09-07・申し込みURLはユーザーから提示予定） |
...
#### フェーズ6-b: 燃料費調整額・再エネ賦課金の自動取得（2026-09-07 追加）

**背景**: フェーズ6（`rate-intake`）は新しい試算表xlsxを人が投入する前提だった。
燃料費調整額・再エネ賦課金は各社が毎月公表する単純な数値であり、xlsxの投入を
待たず**取得から本番反映まで無人で回してよい**とユーザーが判断した
（CLAUDE.md ルール10の一般原則に対する、この2値に限定した例外。詳細はCLAUDE.md参照）。

**設計方針**:
- GitHub Actions の`schedule`（月次cron）で、このリポジトリのCIランナー上から
  各社公式ページを取得する（開発セッションのサンドボックスはネットワークが
  遮断されているため、取得自体はワークフロー実行時に初めて検証できる）
- 取得→パース→`monthlyRates.ts`への追記は、**新しい年月の追加のみ自動反映**する。
  既存の年月と値が食い違う場合（訂正・改定の可能性）は自動反映せず、
  従来どおり人が確認するPRとして止める（SCRAPING_POLICY.mdの「前月比±30%超は
  承認画面へ」という既存方針を、承認画面ではなくPRのdraft化として踏襲）
- 反映はブランチへのコミット→PR作成→CI（build・test・rate-master:check等）が
  緑になったら自動マージ、の順。人の操作を介さないが、既存の品質ゲートは通す
- 取得元のページ構造が変わって解析できない場合は、**推測で値を埋めず失敗させる**
  （ルール8）。ワークフローの失敗自体が気づきの手段になる

**未確定**: 取得元の正確なURL・ページ構造（中国電力の規制料金側燃調ページ、
auでんき公式サイトの燃調ページ）。過去のセッションはネットワーク接続のある環境で
`energia.co.jp`等から裏付けを取れていたが、本セッションでは`EGRESS_BLOCKED`で
確認できていない。ユーザーからURLの提示、またはページ内容の共有を待って
パース処理を実装する。

#### フェーズ10-b: WEB申し込み手続きへの接続（2026-09-07 追加）

試算・商談ナビのその先、**実際の申し込み手続き（各代理店・事業者のWEB申し込みページ）へ
つなぐ**導線を追加する。

**設計方針**:
- 代理事業者ごとにWEB申し込みページのURL・入力項目・遷移が異なるため、
  **プランID・シナリオ単位で申し込み導線を分岐させる**設計とする
  （`ComparisonResult` / `SalesCoach` の「お手続きのご案内」段階から、
  該当プランの申し込みURLへ遷移する形を想定）
- 申し込みページのURL・仕様はユーザーから提示される予定（未着手）
- 個人情報の扱いに注意: 申し込みページへの遷移時に試算結果（使用量・金額等）を
  URLパラメータ等で引き継ぐ設計にする場合、ルール9（個人情報をサーバーに送信しない）
  との整合を確認すること。引き継ぐのは非個人情報（プランID等）に留めるのが無難
...
- **テストカバレッジ**: 100%（calc-core）、80%（Web UI）
  → 達成。calc-core 100% / Web UI 97.9% / scripts 100%。CI が閾値を強制
  （テスト計 817件：calc-core 298 / Web 424 / scripts 95）
