...
「個人情報をサーバーに出すか」の一点だけが判断事項で、出さない案が2つある。

| 案 | 内容 | 人日 | ルール9 |
|---|---|---:|---|
| B-3 AI案内役のみ | 数値は calc-core が出し、文章だけ生成AIに書かせる。送るのは使用量とプランIDだけ | 約18 | 維持 |
| B-2 端末内OCR | ブラウザ内でOCRし画像を送らない。精度は落ちるので確認画面が前提 | 約15 | 維持 |
| B-1 フル | 上表のとおり | 約62 | **改定が必要** |

**推奨は B-3 → B-2 → B-1 の順。** 前2つはルール9 の前提を壊さずに進められる。
