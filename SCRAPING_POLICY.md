...
> **2026-08-20 追記: この文書の位置づけが変わりました**
>
> フェーズ1〜4で、比較対象の単価が **JAでんき公式試算表**（Google Drive で入手可能）から
> 機械的に取り出せることが確定しました。各社サイトをスクレイピングして値を取り直す
> 必要はありません。
>
> さらに、同じプランでも試算表ごとにモデル化が食い違う例が見つかっています
> （夜トクの基本料金半額の扱い、JA従量電灯A 最低料金の 759.68 と 759.67 など）。
> 機械的な取り込みではこの食い違いを解決できず、人が元資料を比べる必要があります。
>
> したがって **スクレイピングは裏取り（cross-check）専用**とし、
> 料金マスターへの反映元にはしません。本文の「人間承認プロセス」は
> [PHASE_6_DESIGN.md](./docs/PHASE_6_DESIGN.md) の Pull Request レビューに読み替えてください。
